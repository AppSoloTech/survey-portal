...
- Phase 25 timing foundation implemented: complete Phase 26 next to replace participant numeric progress labels with remaining-time copy powered by `Survey.effectiveEstimateSeconds`, then Phase 27 for lightweight activity instrumentation and active-time aggregation. Phase 25 intentionally does not implement participant label replacement or dynamic running estimates.
