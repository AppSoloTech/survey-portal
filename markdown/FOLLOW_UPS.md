...
- Performance testing follow-ons: Phase 50 labels Azure Monitor metrics as
  ...
  sample persistence plus read-only APIs only. Phase 53 adds the local capacity
  suite runner, write-time secret omission, linked child run persistence,
  bounded local samples, and aggregate classification. Azure Monitor sampling
  and the Admin suite UI remain deferred to later prompted phases. Decide
  whether suite detail child runs need an explicit cap if suites grow beyond
  the planned small profile set, and add sample cursor/offset support only if
  the Phase 55 viewer needs to page beyond the bounded sample window. Future
  load-test artifact hardening: consider detecting bare JWT-like or secret-like
  strings that have no adjacent key name, URL scheme, bearer prefix, or known
...
- Phase 45/46 public-user manual accessibility verification remains pending for the human browser/assistive-technology pass documented in `markdown/ACCESSIBILITY_TEST_PLAN.md`: keyboard-only dashboard survey actions, category group drill-in and pagination, anonymous survey directory start links, screen reader button/link list spot checks, inline glossary focus/open/dismiss/Escape behavior, account settings reset cooldown regression, contrast spot checks in light/dark themes, mobile screen reader/touch checks, and 375/768/1280px layouts for dashboard, category list, anonymous directory, account settings, and participant survey routes. Codex could not complete this locally because the pass requires real browser and assistive-technology access.
- Optional future accessibility automation: if the human developer approves new dev dependencies, add a small Playwright plus axe smoke suite and `npm run test:accessibility` for representative public/user routes in light and dark themes. Keep it independent from live external services.
